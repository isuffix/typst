...
///
/// Also returns whether the expression was parentheses that were removed due
/// to precedence on their left or right.
fn math_expression(
    ...
    loop {
        ...
        let value = match op.finish {
            ...
            Finish::Delims { left } => {
                let value;
                (value, lhs_deparen) = parse_delimiters(tokens, left, left_parent, mark);
                value
            }

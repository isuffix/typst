...
/// its content (this is similar to the behavior of @str[strings]).
///
/// Raw syntax using _three or more_ backticks is meant to be more flexible and
...
///   containing a group of three backticks, use four or more backticks on
///   either side.
///
/// - *The raw element can be inline or block-level based on linebreaks*
///
///   If the text inside three or more backticks contains a linebreak character,
///   it will produce a block-level element. If the text does not, it will
///   produce an inline element. This sets the @raw.block[`block`] parameter to
...
///   or language tag are whitespace, they are trimmed from the raw text.
...
/// spaces or backticks will still need to be set manually via the

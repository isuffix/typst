...
pub struct LinkedNode<'a> {
    ...
    index: usize,
    // This node's offset into the source file.
    offset: usize,
}
...
impl<'a> LinkedNode<'a> {
    ...
    pub fn children(&self) -> LinkedChildren<'a> {
        LinkedChildren {
            ...
            slice: self.node.children().as_slice(),
            index: 0,
            front: self.offset,
    ...
    /// An iterator over the siblings of this node, starting at the next sibling.
...
impl LinkedNode<'_> {
    ...
    pub fn prev_sibling(&self) -> Option<Self> {
        ...
        children.slice = &children.slice[..self.index];
...
impl<'a> Iterator for LinkedChildren<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let node = self.slice.get(self.index)?;
        let item = LinkedNode {
            node,
...
impl DoubleEndedIterator for LinkedChildren<'_> {
    fn next_back(&mut self) -> Option<Self::Item> {
        let node;
        (node, self.slice) = self.slice.split_last()?;
        self.back -= node.len();
        Some(LinkedNode {
            node,
            parent: Some(self.parent.clone()),

...
impl DiagSpan {
    ...
    /// of an internal span.
    ///
    /// If one of the range's parts exceeds the maximum value of 2^46-1, it is
    /// saturated.
    ...
    ///
    /// The preferred interface for converting a Span into DiagSpan is using
    /// `span.into()`, which calls this with `sub_range: None`.
    pub(crate) fn from_source(span: Span, sub_range: Option<SubRange>) -> Self {

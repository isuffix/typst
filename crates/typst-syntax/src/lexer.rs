...
impl Lexer<'_> {
    ...
    fn raw(&mut self) -> (SyntaxKind, SyntaxNode) {
        ...
        let ticks = self.s.from(end - backticks);
        let inner = Scanner::new(self.s.get(start + backticks..end - backticks));
        let (nodes, tag, diff_future_tag_len) = Self::build_raw_nodes(inner, ticks);
        ...
        if let Some(future_tag_len) = diff_future_tag_len {
            ...
            && start + backticks + tag.len() == end - backticks
        {
            // Empty with no tag/ws is only possible with two backticks, which
            // is handled above.
            ...
            // TODO: Should we hint to add a space or linebreak to avoid the
            // warning?
        }
    ...
    /// Build the raw nodes vector from a scanner over the inner text and a
    /// string of the delimiting backticks.
    fn build_raw_nodes<'a>(
        mut s: Scanner<'a>,
        ticks: &'a str,
    ) -> (Vec<SyntaxNode>, Option<&'a str>, Option<usize>) {
        // Opening delimiter.
        ...
        if ticks.len() >= 3 {
            (tag, diff_future_tag_len) = Self::raw_lang_tag(&mut s);
            if let Some(tag) = tag {
                nodes.push(SyntaxNode::leaf(SyntaxKind::RawLang, tag));
            }
            Self::blocky_raw(&mut s, &mut nodes);
        } else {
            Self::inline_raw(&mut s, &mut nodes);
        }

        // Closing delimiter.
        nodes.push(SyntaxNode::leaf(SyntaxKind::RawDelim, ticks));

        (nodes, tag, diff_future_tag_len)
    }
    ...
    fn raw_lang_tag<'a>(s: &mut Scanner<'a>) -> (Option<&'a str>, Option<usize>) {
        let start = s.cursor();
        // TODO: Change the backtick behavior?
